...
year_district_data <- function(file_path){
  ...
  district_year_list <-  map(district_year_list, ~.x %>%
    mutate(text_length = nchar(text),
           remove_text = ifelse(grepl(paste0("^",format_year), text) & text_length < 25, 1,0)) %>%
    filter(!grepl("^STATE|^As of|^Page", text),
           remove_text == 0) %>%
    select(text, page, y_corrected, school_year))
...
one_district_info <- function(district_df){
 ...
 entry_category_information <- district_df %>%
   ...
   mutate(across(current_year_adm:past_year_admw, ~gsub(",","",.x)),
...
district_level_df <- entry_category_information %>%
  mutate(district_geo = district_details[2],
  ...
  group_by(entry_name) %>%
  mutate(current_year_adm_entry = sum(current_year_adm[category == "ADMr"]),
         ...
         current_weight_entry = ifelse(current_year_effective_admw_entry == current_year_adm_entry,
                                       1, 0)) %>%
  ungroup() %>%
  mutate(
    ...
    current_weight_district = ifelse(current_year_extra_weight_district < 1,
                                     1, 0)
  )
...
# write.csv(admw_district_df[[2]], "data_clean/district_year_admw.csv")
